...
std::vector<int> getNumbersMultiDelim(const std::string &content,
  ...
  return Arda::Parsing::getNumbers(cCopy, delimiter, tokensToConvert);
};
...
std::vector<int> getNumberBlock(std::string content, std::string key) {
  ...
  return Arda::Parsing::getNumbers(bracketBlock, ' ', std::set<int>{});
}
...
int getNumber(const std::string &content) {
  ...
  if (std::regex_search(content, match, pattern)) {
    ...
    Fwg::Utils::Logging::logLine("ERROR: Couldn't extract number from string: ",
                                 content);
